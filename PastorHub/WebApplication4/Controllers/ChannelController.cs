...
namespace WebApplication4.Controllers
{
    public class ChannelController : Controller
    {
        ...
        [HttpPost]
        [Authorize(Roles = "pastor")]
        public ActionResult Create(string Title, IEnumerable<string> Categories, IEnumerable<HttpPostedFileBase> Files, string Text)
        {
            string PastorId = User.Identity.GetUserId();
            Post post = new Post() { Title = Title, Text = Text, Pastor = context.Users.Where(i => i.Id == PastorId).FirstOrDefault(), Date = DateTime.Now};
